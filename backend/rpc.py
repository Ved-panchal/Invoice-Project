...
class RpcClient:
    ...
    def on_response(self, ch, method, props, body):
        print(f'On response: {self.responses}')
        if props.correlation_id in self.responses:
            self.responses[props.correlation_id] = body
            self.responses[props.correlation_id][0].set()

    def call(self, message: dict, callback):
        self.corr_id = str(uuid.uuid4())
        self.responses[self.corr_id] = [threading.Event(), callback]
        ...
        while True:
            self.responses[self.corr_id][0].wait()
            response = self.responses[self.corr_id].pop(1)
            self.responses[self.corr_id][0].clear()
            callback(response)
            if not self.responses[self.corr_id]:
